...
tPage.fields_draw = function(aFieldsCoord)
	...
	for key, tab in pairs(aFieldsCoord) do
		...
		elseif 	tab.type  =="nbit"  					then v_data = (v_data == 1 and "oui"    	or  "non")
		elseif 	tab.field =="start_mode" 				then v_data = (v_data == 0 and "arrete" 	or "lance")
		elseif 	tab.field =="start_top" 				then v_data = (v_data == 0 and "Solo"   	or  (v_data == 1 and "A plusieurs" or "Starter" ))
...
tPage.loadFields = function(aPageIdx)
	...
	then  
		t_fields = {
			[1]={field ="nb_tours",			lx= 2, ly=12, text="Nb tours", 	dx= 50,	type="n+°",		stop=20,	hint = "..."},
			[2]={field ="delay",			lx=70, ly=12, text="Delai", 	dx= 95, type="n+°",		stop=60,	hint = "delai entre 2 validations"},
			[3]={field ="seuil",			lx= 2, ly=22, text="Seuil RSSI",dx= 50, type="n+",		stop=100,	hint = "seuil validation"},
			[4]={field ="start_throttle", 	lx= 2, ly=32, text="Seuil gaz", dx= 50, type="sw_pos",	stop="thr",	hint = "decollage du quad"},
			[5]={field ="start_mode",		lx= 2, ly=42, text="Depart", 	dx= 40, type="n-",		stop=-1,	hint = "au decollage / 1er passage"},
		}
	...
	then  
		t_fields = {
			[1]={field ="start_top",		lx= 2, ly=12, text="Mode", 			dx= 50, type="n+°",		stop=2,		hint = "Top depart"},
			[2]={field ="snd_pilot",		lx= 2, ly=22, text="Pseudo", 		dx= 50, type="nbit",	stop=1,		hint = "annonce nom pilote"},
			[3]={field ="voc_num",			lx= 2, ly=32, text="Num Tour", 		dx= 50, type="nbit",	stop=1,		hint = "annonce tour accompli"},
			[4]={field ="voc_time", 		lx=77, ly=32, text="Temps", 		dx=105, type="nbit",	stop=1,		hint = "annonce des temps"},
			[5]={field ="snd_best", 		lx= 2, ly=42, text="Best", 			dx= 50, type="nbit",	stop=1,		hint = "ann. meilleur tour"},
			--
		...
		return "Chronometre - CONFIG SONS"
	end

	if aPageIdx == SETUP_INTERS
	then  
		t_fields = { 
			[1] = {field ="sw_Arm", 		lx= 2, ly=12, text="Armement",	dx= 55, type="sw_name", stop=0, 			hint = "inter Armement"},
			[2] = {field ="sw_Arm_Off",		lx=80, ly=12, text="Off", 		dx=100, type="sw_pos",  stop="fieldprec", 	hint = "Position Desarmee"},
			[3] = {field ="sw_Lap" ,		lx= 2, ly=22, text="Valid tour",dx= 55, type="sw_name", stop=0, 			hint = "Valid tour manuel"},
			[4] = {field ="sw_Lap_ls_num", 	lx=80, ly=22, text="RSSI",  	dx=105, type="n+°", 	stop=10, 			hint = "inter logique RSSI "},
			[5] = {field ="sw_Reset",		lx= 2, ly=32, text="Reset", 	dx= 55, type="sw_name", stop=0, 			hint = "inter de R.A.Z du chrono"},
			[6] = {field ="sw_Reset_On",	lx=80, ly=32, text="On", 	 	dx=105, type="sw_pos", 	stop="fieldprec", 	hint = "Position RAZ"},
			[7] = {field ="sw_Test",		lx= 2, ly=42, text="Test", 		dx= 55, type="sw_name", stop=0, 			hint = "Tester le seuil RSSI"},
			[8] = {field ="sw_Test_On",		lx=80, ly=42, text="On", 		dx=105, type="sw_pos",  stop="fieldprec", 	hint = "Position TEST"}
		}
